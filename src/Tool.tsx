...
export const Tool = memo(function MyAddonSelector() {
  ...
  useEffect(() => {
    if (tokensCss && global.themeVariableCss) {
      const themeSelected = tokensCss?.themes.filter(
        (_theme: ThemeType) => _theme.name === global.themeVariableCss?.name,
      )[0];
      DisplayToolState("html", {
        isInDocs: false,
        themeVariableCss: global.themeVariableCss.name,
